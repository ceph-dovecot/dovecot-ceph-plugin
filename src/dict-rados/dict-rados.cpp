...
int rados_dict_transaction_commit(struct dict_transaction_context *_ctx, bool async,
  ...
  if (_ctx->changed) {
    ...
    if (ctx->dirty_private) {
      ...
      int err = d->get_io_ctx().aio_operate(d->get_private_oid(), ctx->completion_private, &ctx->write_op_private);
    ...
    if (ctx->dirty_shared) {
      ...
      int err = d->get_io_ctx().aio_operate(d->get_shared_oid(), ctx->completion_shared, &ctx->write_op_shared);
    ...
    if (!failed) {
      if (!async) {
        ...
        if (callback != NULL) {
          callback(failed ? RADOS_COMMIT_RET_FAILED : RADOS_COMMIT_RET_OK, context);
        }
