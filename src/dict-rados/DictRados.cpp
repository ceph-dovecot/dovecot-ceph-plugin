...
public:
	...
	rados_dict_transaction_context() {
		...
		dirty_shared = false;
		completion_shared = librados::Rados::aio_create_completion();
		callback = nullptr;
...
int rados_transaction_commit(struct dict_transaction_context *_ctx, bool async, dict_transaction_commit_callback_t *callback,
	...
	if (_ctx->changed) {
		...
		if (!failed && ctx->dirty_private) {
			...
			failed = d->get_private_ctx().aio_operate(d->get_private_oid(), ctx->completion_private, &ctx->write_op_private) < 0;
		}

		if (!failed && ctx->dirty_shared) {
			i_debug("rados_transaction_commit() operate(%s)", d->get_shared_oid().c_str());
			failed = d->get_shared_ctx().aio_operate(d->get_shared_oid(), ctx->completion_shared, &ctx->write_op_shared) < 0;
		}
