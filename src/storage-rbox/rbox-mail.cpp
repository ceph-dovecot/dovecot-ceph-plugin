...
static int rbox_mail_get_received_date(struct mail *_mail, time_t *date_r) {
  ...
  if (value == NULL) {
    ...
    if (value == NULL) {
      // file exists but receive date is unknown, due to missing index entry and missing
      // RADOS x-attribute
      i_error("receive_date for object(%s) is not in index and not in x-attributes!",
              rmail->rados_mail->get_oid()->c_str());
      return -1;
    }
  } else {
    // value is ptr to bufferlist.buf we do not neet to free it here
    free_value = false;
  }
  try {
...
static int rbox_mail_get_stream(struct mail *_mail, bool get_body ATTR_UNUSED, struct message_size *hdr_size,
  ...
  if (data->stream == NULL) {
    ...
    // we could also use get_physical size and get save date for this, but this way, it's much faster.
    if (index_mail_get_physical_size(_mail, &psize) < 0 || index_mail_get_save_date(_mail, &save_date) < 0) {
      read_mail_op.stat(&psize, &save_date, &stat_err);
    }
    // we need the results of the operations immediately.
    ret = rados_storage->get_io_ctx().operate(*rmail->rados_mail->get_oid(), &read_mail_op,
    ...
    if (get_mail_stream(rmail, rmail->rados_mail->get_mail_buffer(), psize, &input) < 0) {
      FUNC_END_RET("ret == -1");
      delete rmail->rados_mail->get_mail_buffer();
      return -1;
    }
