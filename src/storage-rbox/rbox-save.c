...
{
	struct rbox_save_context *ctx =
		(struct rbox_save_context *)t->save_ctx;
					 ...
					 struct dbox_file *file)
{
	...
	if (count > 0) {
		sfile->attachment_pool =
			pool_alloconly_create("rbox attachment paths", 512);
...
static int dbox_save_assign_uids(struct rbox_save_context *ctx,
...
int rbox_transaction_save_commit_pre(struct mail_save_context *_ctx)
{
	...
	if (rbox_sync_begin(ctx->mbox, RBOX_SYNC_FLAG_FORCE |
			     RBOX_SYNC_FLAG_FSYNC, &ctx->sync_ctx) < 0) {
	...
	dbox_save_update_header_flags(&ctx->ctx, ctx->sync_ctx->sync_view,
		ctx->mbox->hdr_ext_id, offsetof(struct rbox_index_header, flags));
