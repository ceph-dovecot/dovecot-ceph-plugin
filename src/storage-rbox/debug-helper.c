...
const char *rbox_unixdate2str(time_t timestamp) { return t_strflocaltime("%Y-%m-%d %H:%M:%S", timestamp); }
...
void rbox_dbg_print_mailbox_list_settings(struct mailbox_list_settings *target, const char *funcname,
                                          const char *name) {
