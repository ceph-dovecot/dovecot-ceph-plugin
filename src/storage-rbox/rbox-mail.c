...
static void rbox_mail_set_expunged(struct dbox_mail *mail)
{
	...
	mail_storage_set_critical(_mail->box->storage,
				  "rbox %s: Unexpectedly lost uid=%u",
...
static int rbox_mail_file_set(struct dbox_mail *mail)
{
	...
	if (mail->open_file != NULL) {
		...
		mail->open_file =
			rbox_save_file_get_file(_mail->transaction,
						 _mail->seq);
		...
		if (ret <= 0) {
			mail_storage_set_critical(_mail->box->storage,
				"rbox %s: Unexpectedly lost mail being saved",
...
static int
rbox_mail_get_special(struct mail *_mail, enum mail_fetch_field field,
		       const char **value_r)
{
	struct rbox_mailbox *mbox = (struct rbox_mailbox *)_mail->box;
...
int rbox_mail_open(struct dbox_mail *mail, uoff_t *offset_r,
