			    ...
			    struct rbox_file *dest_file)
{
	...
	dest_file->attachment_pool =
		pool_alloconly_create("rbox attachment copy paths", 512);
	...
	array_foreach(&extrefs, extref) T_BEGIN {
		src = t_strdup_printf("%s/%s", dest_storage->attachment_dir,
			rbox_file_attachment_relpath(src_file, extref->path));
...
rbox_copy_hardlink(struct mail_save_context *_ctx, struct mail *mail)
{
	...
	ret = rbox_file_copy_attachments((struct rbox_file *)src_file,
					  (struct rbox_file *)dest_file);
	if (ret <= 0) {
...
int rbox_copy(struct mail_save_context *_ctx, struct mail *mail)
{
	...
	if (mail_storage_copy_can_use_hardlink(mail->box, &mbox->box) &&
	    _ctx->data.guid == NULL) {
		T_BEGIN {
			ret = rbox_copy_hardlink(_ctx, mail);
		} T_END;
