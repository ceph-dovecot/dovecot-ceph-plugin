...
static int set_user_env(struct mail_user *user, const char *val) {
  ...
  i_debug("set_user_env called!!!!");
  if (!array_is_created(&user->set->plugin_envs)) {
    i_array_init(&user->set->plugin_envs, 4);
    newly_created = true;
  }
  ...
  array_append(&user->set->plugin_envs, &user_env_save_continue, 1);
  array_append(&user->set->plugin_envs, &user_env_save_continue_value, 1);
