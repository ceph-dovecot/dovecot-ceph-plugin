...
TEST_F(SyncTest, force_resync_missing_rados_object) {
  ...
  ASSERT_NE(ns, nullptr);
  struct mailbox *box = mailbox_alloc(ns->list, "INBOX", MAILBOX_FLAG_IGNORE_ACLS);

  if (mailbox_open(box) < 0) {
    i_error("Opening mailbox %s failed: %s", "INBOX", mailbox_get_last_internal_error(box, NULL));
    ...
    if (mailbox_sync(box, MAILBOX_SYNC_FLAG_FORCE_RESYNC | MAILBOX_SYNC_FLAG_FIX_INCONSISTENT) < 0) {
      i_error("Forcing a resync on mailbox %s failed: %s", "INBOX", mailbox_get_last_internal_error(box, NULL));
      FAIL() << " Forcing a resync on mailbox INBOX Failed";
    }
